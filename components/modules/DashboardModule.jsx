...
// ══════════════════════════════════════════════════════════════
export default function DashboardModule({ students: studentsProp, finRecords: finRecordsProp, settings }) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
        ...
        <KPICard icon="📉" label="إجمالي الديون" value={fmtM(students.reduce((a, s) => a + (s.totalFees - s.paid), 0))} sub="ج.م" color="#f87171" />
      </div>
      <ProblemSection data={dd.expensesSection} idRef={refs.expenses} />
      <ProblemSection data={dd.absenceSection}  idRef={refs.absence}  />
      <ProblemSection data={dd.examsSection}     idRef={refs.exams}    />
