...
export default function FinanceModule({ students, settings, setSettings, finRecords, setFinRecords, setStudents, addActivity, role = "admin", jumpTo, onJumpDone }) {
  ...
  // ── تقرير التحصيل اليومي (مستطيل جديد — المستر فقط) ──────────────
  ...
  // ── تقرير التحصيل اليومي: كل من دفع في يوم/شهر محدد، بغض النظر عن السجل الرئيسي فوق ──
  ...
  return (
    <div className="space-y-4">
      ...
      {!isAssist && (
        <div className="bg-slate-800/60 border border-slate-700/40 rounded-2xl p-4 space-y-3">
          <div className="text-xs text-slate-400 font-bold mb-1">📅 تقرير تحصيل يوم محدد (المستر فقط)</div>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
            <Field label="الصف">
          ...
          {dTableOpen && (
            dayRecords.length === 0
              ? <div className="text-center py-6 text-slate-600 text-sm">📭 محدش دفع في يوم {dSelDay} {MONTHS_AR[dSelMonth - 1]}</div>
