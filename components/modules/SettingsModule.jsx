    ...
    <div className="fixed inset-0 bg-black/70 z-[100] flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-slate-900 border border-slate-700/60 rounded-3xl w-full max-w-md max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
        ...
        <div className="p-5">
              ...
              <div className="space-y-2">
                ...
                <button onClick={() => setView("cashierpwd")} className="w-full flex items-center gap-3 px-4 py-4 rounded-2xl bg-slate-800/50 hover:bg-slate-800 transition-colors text-right">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-slate-500 text-xs">{settings.cashierPassword ? "✓ مُعيَّنة — دخول لتعديل كلمة السر" : "⚠️ غير مُعيَّنة — دخول لتعيين كلمة السر"}</div>
                  </div>
                  <span className="text-slate-600 shrink-0">←</span>
                </button>
              </div>
          ...
          {view === "password" && (() => { const [cp, setCp] = useState(""); const [np, setNp] = useState(""); const [cf, setCf] = useState(""); const [er, setEr] = useState(""); return <><Back to="passwords" /><div className="space-y-4"><Field label="كلمة المرور الحالية"><Inp type="password" value={oldP} onChange={e => { setOldP(e.target.value); setPErr(""); }} /></Field><Field label="الجديدة"><Inp type="password" value={newP} onChange={e => { setNewP(e.target.value); setPErr(""); }} /></Field><Field label="تأكيد"><Inp type="password" value={confP} onChange={e => { setConfP(e.target.value); setPErr(""); }} /></Field>{pErr && <div className="bg-red-500/10 border border-red-500/20 rounded-xl px-3 py-2.5 text-red-400 text-sm">⚠️ {pErr}</div>}<Btn variant="primary" size="lg" className="w-full" onClick={changePwd}>🔑 تغيير كلمة سر المستر</Btn></div></>; })()}
          {view === "cashierpwd" && (() => { const [np, setNp] = useState(""); const [cf, setCf] = useState(""); const [er, setEr] = useState(""); return <><Back to="passwords" /><div className="space-y-4"><div className="bg-amber-500/10 border border-amber-500/20 rounded-xl px-3 py-2 text-amber-400 text-xs">💼 كلمة سر الاسيست — تُستخدم عند دخول دور Assist</div><Field label="كلمة المرور الجديدة"><Inp type="password" value={np} onChange={e => { setNp(e.target.value); setEr(""); }} /></Field><Field label="تأكيد"><Inp type="password" value={cf} onChange={e => { setCf(e.target.value); setEr(""); }} /></Field>{er && <div className="bg-red-500/10 border border-red-500/20 rounded-xl px-3 py-2.5 text-red-400 text-sm">⚠️ {er}</div>}<Btn variant="primary" size="lg" className="w-full" onClick={() => changeRolePwd("الاسيست","cashierPassword",np,cf,setEr)}>💾 حفظ كلمة سر الاسيست</Btn></div></>; })()}
