      ...
      <div className="bg-slate-900 border border-slate-700/60 rounded-3xl w-full max-w-md max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
        ...
        <div className="p-5">
          ...
          {view === "backup" && <><Back /><div className="space-y-4">
            {/* ── #LiveSync: حالة المزامنة اللحظية التلقائية + شريط استخدام
                Firestore (نسبة حجم مستند المزامنة من أقصى حد مسموح) ──*/}
            <div className="bg-emerald-500/10 border border-emerald-500/20 rounded-2xl p-4 space-y-2">
              <div className="text-emerald-400 font-bold text-sm">🔄 المزامنة التلقائية اللحظية</div>
              <div className={`text-xs font-bold rounded-xl px-3 py-2 ${liveSyncState?.status === "error" ? "bg-red-500/10 text-red-400" : liveSyncState?.status === "success" ? "bg-emerald-500/10 text-emerald-300" : "bg-slate-800/60 text-slate-400"}`}>
              ...
              {firestoreUsagePct > 0 && (
                <div className="space-y-1 pt-1">
                  <div className="flex items-center justify-between text-[11px] font-bold">
                    <span className={firestoreUsagePct >= 80 ? "text-red-400" : firestoreUsagePct >= 50 ? "text-amber-400" : "text-slate-400"}>مساحة بيانات المزامنة</span>
                    <span className={firestoreUsagePct >= 80 ? "text-red-400" : firestoreUsagePct >= 50 ? "text-amber-400" : "text-slate-400"}>{firestoreUsagePct}%</span>
                  </div>
                  <div className="w-full h-2 rounded-full bg-slate-800/80 overflow-hidden">
                    <div className={`h-full rounded-full transition-all ${firestoreUsagePct >= 80 ? "bg-red-500" : firestoreUsagePct >= 50 ? "bg-amber-500" : "bg-emerald-500"}`} style={{ width: `${firestoreUsagePct}%` }} />
                  </div>
                  {firestoreUsagePct >= 80 && (
                    <div className="text-red-400 text-[11px] font-bold">⚠️ قربت توصلي للحد الأقصى المسموح — كلمي Claude يظبطلك حل</div>
                  )}
                </div>
              )}
            </div>
