...
export default function SettingsModule({ settings, setSettings, students, setStudents, finRecords, setFinRecords, webExams, setWebExams, centerExams, setCenterExams, examQs, setExamQs, cloudBackupState, backupToCloud, restoreFromCloud, addActivity, onClose }) {
  ...
  const changeRolePwd = async (roleKey, pwdKey, newVal, confirmVal, setErrFn) => {
    ...
    save(pwdKey, hashed, `✓ تم تغيير كلمة مرور ${roleKey}`); setErrFn(""); setCaP(""); setCaConfP(""); setView("main");
  };
  const addWa = () => {
    ...
    <div className="fixed inset-0 bg-black/70 z-[100] flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-slate-900 border border-slate-700/60 rounded-3xl w-full max-w-md max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
        ...
        <div className="p-5">
          ...
          {view === "password" && <><Back to="passwords" /><div className="space-y-4"><Field label="كلمة المرور الحالية"><Inp type="password" value={oldP} onChange={e => { setOldP(e.target.value); setPErr(""); }} /></Field><Field label="الجديدة"><Inp type="password" value={newP} onChange={e => { setNewP(e.target.value); setPErr(""); }} /></Field><Field label="تأكيد"><Inp type="password" value={confP} onChange={e => { setConfP(e.target.value); setPErr(""); }} /></Field>{pErr && <div className="bg-red-500/10 border border-red-500/20 rounded-xl px-3 py-2.5 text-red-400 text-sm">⚠️ {pErr}</div>}<Btn variant="primary" size="lg" className="w-full" onClick={changePwd}>🔑 تغيير كلمة سر المستر</Btn></div></>}
