...
export default function App() {
  ...
  const {
    ...
    liveSyncState, firestoreUsagePct, // #LiveSync
  ...
  if (!currentRole) {
    return <RoleGate settings={settings} onEnter={(r) => {
      setCurrentRole(r);
      if (r.role === "assist") {
        setPage("dashboard");
        showToast(`👋 أهلا مس ${r.name || r.label}`, "success");
      }
      addActivity("login", `دخل كـ ${r.label}`);
    }} />;
  }
  ...
  return (
    <div className="flex h-screen overflow-hidden nova-bg" dir="rtl"
      ...
      <div className="relative z-10 flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        <main className="flex-1 overflow-y-auto p-4">
          <div className="max-w-2xl mx-auto">
            ...
            {safePage === "students"   && <StudentsModule   students={students || []} setStudents={setStudents} finRecords={finRecords || []} setFinRecords={setFinRecords} attRecords={attRecords || []} setAttRecords={setAttRecords} webExams={webExams || []} centerExams={centerExams || []} settings={settings} role={currentRole.role} jumpTo={jumpStudent} onJumpDone={() => setJumpStudent(null)} addActivity={addActivity} />}
      ...
      {showSettings && (
        <Suspense fallback={<ModuleLoading />}>
          <SettingsModule settings={settings} setSettings={setSettings} students={students || []} setStudents={setStudents} finRecords={finRecords || []} setFinRecords={setFinRecords} webExams={webExams || []} setWebExams={setWebExams} centerExams={centerExams || []} setCenterExams={setCenterExams} examQs={examQs || []} setExamQs={setExamQs} cloudBackupState={cloudBackupState} backupToCloud={backupToCloud} restoreFromCloud={restoreFromCloud} liveSyncState={liveSyncState} firestoreUsagePct={firestoreUsagePct} addActivity={addActivity} onClose={() => setShowSettings(false)} />
        </Suspense>
      )}
